...
import ListSettings from "../../../../../../frappe/frappe/public/js/frappe/list/list_settings";
		...
		return `
			<div class="list-row-container" tabindex="1" >
				<table border="0" width="100%">
					<tr class="d-flex" style="padding-left:15px !important;">
						<td class="level-left ellipsis hidden-xs" widht="90%" >
							<p>${left}<p>
						</td>
						<td class="level-right text-muted ellipsis" widht="10%" align="left">
							<p>${right}<p>
						</td>
					<tr>
				</table>
	...
	get_column_html(col, doc) {
		...
		const field_html = () => {
			...
			if (df.fieldtype === "Image") {
				...
				html = `<a class="filterable ellipsis"
					data-filter="${fieldname},=,${frappe.utils.escape_html()}">
					${format()}
				</a>`;
			}
			// var all_values = frappe.utils.escape_html(_value);
			// var stringLength = all_values.length;
